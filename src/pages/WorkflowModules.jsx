...
export function TripWorkflow({ action = 'listing', tripId }) {
  ...
  if (action === 'create') return <div className="space-y-4"><TripNavigation /><Card title="Create Trip Request"><form className="grid gap-3 md:grid-cols-2" onSubmit={(e) => { e.preventDefault(); create.mutate(form); }}>
    ...
    <label><span className="label">Suggested trip</span><select className="input" value={suggestedEmployeeId} onChange={(event) => setSuggestedEmployeeId(event.target.value)}><option value="">Select employee</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></label>
